...
class RuleEngine:
        ...
        try:
            ...
            payload = {
                ...
                "metadata": {
                    ...
                    "created_rule_prompts": [],
                },
            }
        payload.setdefault("erp_schema", {})
        payload.setdefault("post_transformation_actions", {})
        metadata = payload.setdefault("metadata", {})
        metadata.setdefault("crm_columns", [])
        metadata.setdefault("erp_system", "Odoo")
        metadata.setdefault("version", "1.0.0")
        metadata.setdefault("last_updated", datetime.utcnow().isoformat())
        metadata.setdefault("created_rule_prompts", [])
        return payload
    ...
    def reload_schema(self) -> None:
        """Reload schema from disk so chat sees external edits immediately."""
        self.schema = self._load_schema()

    def _get_created_rule_prompts(self) -> list[dict]:
        metadata = self.schema.setdefault("metadata", {})
        prompts = metadata.setdefault("created_rule_prompts", [])
        if not isinstance(prompts, list):
            metadata["created_rule_prompts"] = []
            prompts = metadata["created_rule_prompts"]
        return prompts

    def log_created_rule_prompt(self, prompt: str, erp_column: str, rule_name: str) -> None:
        prompts = self._get_created_rule_prompts()
        prompts.append(
            {
    ...
    def _remove_created_rule_prompts(self, erp_column: str, rule_name: str) -> None:
        prompts = self._get_created_rule_prompts()
        filtered_prompts = [
            item
            for item in prompts
            if not (
                ...
                and str(item.get("erp_column", "")).strip() == erp_column
                and str(item.get("rule_name", "")).strip() == rule_name
            )
        ]
        if len(filtered_prompts) != len(prompts):
            self.schema.setdefault("metadata", {})["created_rule_prompts"] = filtered_prompts

    def _get_active_created_rule_prompts(
        self, erp_column: Optional[str] = None
    ) -> list[dict]:
        prompts = self._get_created_rule_prompts()
        ...
        for item in prompts:
            if not isinstance(item, dict):
                continue
            current_column = str(item.get("erp_column", "")).strip()
            current_rule = str(item.get("rule_name", "")).strip()
            if not current_column or not current_rule:
                continue
            if current_column not in self.schema.get("erp_schema", {}):
                continue
            rules = self.schema["erp_schema"][current_column].get("rules", {})
            if current_rule not in rules:
                continue
            active_prompts.append(item)

        if len(active_prompts) != len(prompts):
            self.schema.setdefault("metadata", {})["created_rule_prompts"] = active_prompts
            self._save_schema()
        ...
        if erp_column:
            return [
                ...
                if str(item.get("erp_column", "")).strip() == erp_column
            ]
    ...
    def _conditions_are_duplicate(
        self, existing_conditions: list, new_conditions: list
    ) -> bool:
        """
        ...
        for ec, nc in zip(existing_conditions, new_conditions):
            if (
                ec.get("crm_column") != nc.get("crm_column")
                or ec.get("operator") != nc.get("operator")
                or ec.get("value") != nc.get("value")
            ):
                return False
        return True
    ...
    def add_rule(self, erp_column: str, rule_name: str, rule_data: Dict) -> Dict:
        ...
        for existing_rule_name, existing_rule_data in existing_rules.items():
            if self._conditions_are_duplicate(
                existing_rule_data.get("conditions", []), new_conditions
            ):
                # Same conditions found — update instead of creating duplicate
                existing_rule_data["conditions"] = new_conditions
                existing_rule_data["priority"] = rule_data.get(
                    "priority", existing_rule_data.get("priority", 1)
                )
                existing_rule_data["enabled"] = rule_data.get(
                    "enabled", existing_rule_data.get("enabled", True)
                )
                existing_rule_data["notes"] = rule_data.get(
                    "notes", existing_rule_data.get("notes", "")
                )
    ...
    def delete_rule(self, erp_column: str, rule_name: str) -> Dict:
        ...
        del rules[rule_name]
        self._remove_created_rule_prompts(erp_column=erp_column, rule_name=rule_name)
        self._save_schema()
    ...
    def list_rules(
        self, erp_column: Optional[str] = None, enabled_only: bool = False
    ) -> Dict:
        ...
        if erp_column:
            ...
            sorted_rules = dict(
                sorted(rules.items(), key=lambda x: x[1].get("priority", 999))
            )
            ...
            return {
                ...
                "created_rule_prompts": self._get_active_created_rule_prompts(
                    erp_column=erp_column
                ),
            }
        else:
            ...
            for rule_name, rule_data in col_data.get("rules", {}).items():
                ...
                if "conditions" in search_in:
                    for condition in rule_data.get("conditions", []):
                        if (
                            search_term_lower
                            in str(condition.get("crm_column", "")).lower()
                            or search_term_lower
                            in str(condition.get("value", "")).lower()
                        ):
                            match = True
                            break
                ...
                if "transformation" in search_in:
                    ...
                    if (
                        search_term_lower in str(transform.get("value", "")).lower()
                        or search_term_lower
                        in str(transform.get("formula", "")).lower()
                    ):
                        match = True
    ...
    def get_summary(self) -> Dict:
        ...
        for col_data in self.schema["erp_schema"].values():
            ...
            if rules:
                ...
                enabled_rules += sum(
                    1 for r in rules.values() if r.get("enabled", True)
                )
        ...
        return {
            ...
            "summary": {
                ...
                "crm_columns_available": len(
                    self.schema["metadata"].get("crm_columns", [])
                ),
        ...
        for col_name, col_data in self.schema["erp_schema"].items():
            ...
            for rule_name, rule_data in col_data.get("rules", {}).items():
                ...
                if "transformation" not in rule_data:
                    errors.append(
                        f"Rule '{col_name}.{rule_name}' missing transformation"
                    )
...
- OR: At least one condition must be true"""
    ...
    **Examples:**
    ...
    def process_request(self, user_input: str) -> Dict:
        ...
        try:
            self.rule_engine.reload_schema()
    ...
    def _execute_operation(self, llm_response: Dict, original_input: str) -> Dict:
        ...
        try:
            if operation == "add_rule":
                result = self.rule_engine.add_rule(
                    erp_column=llm_response.get("erp_column"),
                    rule_name=llm_response.get("rule_name"),
                    rule_data=llm_response.get("rule_data", {}),
                )
                if result.get("status") == "success":
                    self.rule_engine.log_created_rule_prompt(
                        prompt=original_input,
                        erp_column=str(result.get("erp_column", "")),
                        rule_name=str(result.get("rule_name", "")),
                    )
                return result
                ...
                input_lc = original_input.lower()
                has_rule_payload = bool(llm_response.get("erp_column")) or bool(
                    llm_response.get("rule_data")
                ) or bool(llm_response.get("updates"))
                mentions_rule = bool(re.search(r"\brules?\b", input_lc))
                has_rule_action_word = bool(
                    re.search(
                        r"\b(add|create|update|delete|view|list|show|search|get|display)\b",
                        input_lc,
                    )
                )
                looks_like_rule_intent = mentions_rule and (
                    has_rule_action_word or has_rule_payload
                )
                available_columns = sorted(
                    list(self.rule_engine.schema.get("erp_schema", {}).keys())
                )

                if looks_like_rule_intent:
                    detected_column = None

                    erp_column_candidate = str(llm_response.get("erp_column", "")).strip()
                    if erp_column_candidate:
                        detected_column = re.sub(
                            r"[^a-z0-9_]+", "_", erp_column_candidate.lower()
                        ).strip("_")
                    else:
                        match = re.search(
                            r"\bin\s+([A-Za-z0-9_ ]+?)(?:\s*:|\s+where\b|\s+if\b|$)",
                            original_input,
                            re.IGNORECASE,
                        )
                        if match:
                            detected_column = re.sub(
                                r"[^a-z0-9_]+", "_", match.group(1).strip().lower()
                            ).strip("_")

                    if (
                        detected_column
                        and detected_column not in self.rule_engine.schema.get("erp_schema", {})
                    ):
                        return {
                            "status": "error",
                            "message": (
                                f"ERP column '{detected_column}' does not exist. "
                    ...
                    return {
                        "status": "error",
                        "message": (
                            "Rule request understood, but I need an existing ERP column target. "
                            f"Available columns: {available_columns}"
                        ),
                    }

                if not mentions_rule:
                    return {
                        "status": "not_allowed",
                        "message": (
                            "I can help only with transformation rule operations. "
                            "Try: add, update, delete, list, search, or summarize rules."
                        ),
                    }
...
def create_llm_manager(
    *,
    ...
    return LLMRuleManager(engine, api_key=resolved_api_key, model=model)


if __name__ == "__main__":
    manager = create_llm_manager()
    sample_request = "add rule for acme trading customer name set tax to 20%"
    result = manager.process_request(sample_request)
